"""
Functions to:
- register a user: auth_register_v1
- login a user: auth_login_v1
"""
...
def auth_login_v1(email, password):
    ...
    return {'auth_user_id': auth_user['id'],}


def auth_register_v1(email, password, name_first, name_last):
    """
    Given a users email, password, first name and last name, generate a handle and
    ...
    if re.match(r'^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}$', email) and len(password) >= 6\
        and 1 <= len(name_first) <= 50 and 1 <= len(name_last) <= 50:

        store = data_store.get()

        users = store['users']

        permission = 1 if len(users) == 0 else 2
