import pytest
from src.server import user_profile
...
def test_removed_from_channels_dms(clear_and_register):
    ...
    rh.channels_create(clear_and_register, "channel2", True)
    rh.channel_join(clear_and_register, channel1_id)
    # dm1_id = rh.dm_create(clear_and_register, [user2_id]).json()["dm_id"]
    # rh.dm_create(clear_and_register, [rh.auth_login("random@gmail.com", "123abc!@#").json()["auth_user_id"]])
    ...
    assert len(channel1_details["owner_members"]) == 0
...
def test_removed_users_profile(clear_and_register):
    ...
    rh.admin_user_remove(clear_and_register, user2_id)
    response = rh.user_profile(clear_and_register, user2_id)
    assert response.status_code == 200
...
def test_reuseable_email_handle(clear_and_register):
    ...
    assert user2_profile["user"]["email"] == user3_profile["user"]["email"]
    assert user2_profile["user"]["handle_str"] == user3_profile["user"]["handle_str"]
...
def test_remove_global_user():
    ...
    rh.admin_userpermission_change(first_owner["token"], second_owner["auth_user_id"], 1)
...
def test_remove_only_global_user(clear_and_register):
    ...
    response = rh.admin_user_remove(clear_and_register, user1_id)
    assert response.status_code == 400
...
def test_not_authorised_user():
    ...
    user1_token = rh.auth_register("random2@gmail.com", "123abc!@#", "Dan", "Smith").json()["token"]
    user2_id = rh.auth_register("random3@gmail.com", "123abc!@#", "Bob", "Smith").json()["auth_user_id"]
...
# Removing already removed user
def test_removing_removed_user(clear_and_register):
    user2_id = rh.auth_register("random2@gmail.com", "123abc!@#", "Bob", "Smith").json()["auth_user_id"]
    rh.admin_user_remove(clear_and_register, user2_id)
    response = rh.admin_user_remove(clear_and_register, user2_id)
    assert response.status_code == 400
