...
def test_invalid_channel(clear, user1, user2):
    ...
    ban_response = rh.channel_ban(user1["token"], int(channel_id + 1), user2["auth_user_id"])
    assert ban_response.status_code == 400

def test_invalid_user_ban(clear, user1, user2):
...
def test_self_banning(clear, user1):
    channel_id = create_channel(user1["token"], [], True)
    ban_response = rh.channel_ban(user1["token"], channel_id, user1["auth_user_id"])
    assert ban_response.status_code == 400
...
def test_owner_ban_owner(clear, user1, user2):
    ...
    ban_response = rh.channel_ban(user2["token"], channel_id, user1["auth_user_id"])
    assert ban_response.status_code == 400   
    ban_response = rh.channel_ban(user1["token"], channel_id, user2["auth_user_id"])
    assert ban_response.status_code == 400
...
def test_unauthorised_user(clear, user1, user2, user3):
    channel_id = create_channel(user1["token"], [user2["token"]], True)
    ban_response = rh.channel_ban(user3["token"], channel_id, user2["auth_user_id"])
...
def test_valid_bans_in_public(clear, user1, user2, user3):
    ...
    ban_response = rh.channel_ban(user1["token"], channel_id, user3["auth_user_id"])
    assert ban_response.status_code == 200
