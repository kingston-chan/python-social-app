...
# ==== Helper Functions ==== #
def create_channel(creater_token, invited_members_token):
...
def test_invalid_channel(clear, user1, user2):
    channel_id = create_channel(user1["token"], [user2["token"]])
    kick_response = rh.channel_kick(user1["token"], int(channel_id + 1), user2["auth_user_id"])
    assert kick_response.status_code == 400

def test_invalid_user_kicked(clear, user1):
    channel_id = create_channel(user1["token"], [])
    kick_response = rh.channel_kick(user1["token"], channel_id, 99999)
    assert kick_response.status_code == 400

def test_self_kicking(clear, user1):
    channel_id = create_channel(user1["token"], [])
    kick_response = rh.channel_kick(user1["token"], channel_id, user1["auth_user_id"])
    assert kick_response.status_code == 400
...
def test_owner_kick_owner(clear, user1, user2):
    ...
    kick_response = rh.channel_kick(user2["token"], channel_id, user1["auth_user_id"])
    assert kick_response.status_code == 400   
    kick_response = rh.channel_kick(user1["token"], channel_id, user2["auth_user_id"])
    assert kick_response.status_code == 400

## Access Error - 403 ##
def test_unauthorised_user(clear, user1, user2, user3):
    channel_id = create_channel(user1["token"], [user2["token"]])
    kick_response = rh.channel_kick(user3["token"], channel_id, user2["auth_user_id"])
...
def test_valid_kicks(clear, user1, user2, user3):
    channel_id = create_channel(user1["token"], [user2["token"], user3["token"]])
    ...
    kick_response = rh.channel_kick(user1["token"], channel_id, user3["auth_user_id"])
    assert kick_response.status_code == 200
