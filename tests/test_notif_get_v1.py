...
def test_fail_channel_msg(clear,user1,user2):
    ...
    message_id = rh.message_send(user1["token"], channel["channel_id"], "{} Hello".format('@' + rh.user_profile(user2["token"],user2["auth_user_id"]).json()["user"]["handle_str"])).json()["message_id"]
...
def test_fail_dm_msg(clear,user1,user2):
    ...
    message_id = rh.message_senddm(user1["token"], dm["dm_id"], "{} Hello".format('@' + rh.user_profile(user2["token"],user2["auth_user_id"]).json()["user"]["handle_str"])).json()["message_id"]
