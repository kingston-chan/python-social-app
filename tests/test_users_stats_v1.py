...
def test_first_user_registers(clear_and_register):
    ...
    assert abs(workspace_1["channels_exist"][-1]["time_stamp"] - time_stamp) < 2
    assert len(workspace_1["dms_exist"]) == 1
    assert workspace_1["dms_exist"][-1]["num_dms_exist"] == 0
    assert abs(workspace_1["dms_exist"][-1]["time_stamp"] - time_stamp) < 2
    assert len(workspace_1["messages_exist"]) == 1
    assert workspace_1["messages_exist"][-1]["num_messages_exist"] == 0
    assert abs(workspace_1["messages_exist"][-1]["time_stamp"] - time_stamp) < 2
    ...
    assert len(workspace_1["channels_exist"]) == 1
    assert abs(workspace_2["channels_exist"][-1]["time_stamp"] - time_stamp) < 2
    assert len(workspace_1["dms_exist"]) == 1
    assert abs(workspace_2["dms_exist"][-1]["time_stamp"] - time_stamp) < 2
    assert len(workspace_1["messages_exist"]) == 1
    assert abs(workspace_2["messages_exist"][-1]["time_stamp"] - time_stamp) < 2
...
def test_first_user_join_channel_dm(clear_and_register):
    ...
    assert abs(workspace["channels_exist"][-1]["time_stamp"] - time_stamp) < 2

    assert len(workspace["dms_exist"]) == 2
    assert workspace["dms_exist"][-1]["num_dms_exist"] == 1
    assert abs(workspace["dms_exist"][-1]["time_stamp"] - time_stamp) < 2
...
def test_workspace_messages_metrics(clear_and_register):
    ...
    assert abs(workspace["messages_exist"][0]["num_messages_exist"] - time_stamp) < 2
    assert abs(workspace["messages_exist"][1]["num_messages_exist"] - time_stamp) < 2
    assert abs(workspace["messages_exist"][2]["num_messages_exist"] - time_stamp) < 2
    assert abs(workspace["messages_exist"][3]["num_messages_exist"] - time_stamp) < 2
    assert abs(workspace["messages_exist"][4]["num_messages_exist"] - time_stamp) < 2
